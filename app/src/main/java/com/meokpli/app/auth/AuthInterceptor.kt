...
class AuthInterceptor(private val tokenManager: TokenManager) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val urlPath = original.url.encodedPath // 요청 경로 확인

        val builder = original.newBuilder()

        // /reissue 요청에는 Authorization 헤더를 붙이지 않음
        if (!access.isNullOrEmpty() && !urlPath.endsWith("/reissue")) {
...
단, /reissue 요청은 만료된 access token으로 막히지 않도록 Authorization 헤더 제외.
